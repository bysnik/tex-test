...
\begin{center}
  ...
  {\bfseries \input{discipline1.txt}}\\
  {\bfseries \input{code.txt} \input{speciality.txt}}
\end{center}
...
КОС промежуточной аттестации предназначены для контроля и оценки образовательных достижений студентов, проходящих \input{discipline2.txt}\unskip{}.

КОС разработаны в соответствии требованиями ООП СПО по профессии \input{code.txt}\unskip{}, квалификации Сетевой и системный администратор, рабочей программы МДК.
...
По результатам изучения \input{discipline2.txt}\unskip{} студент должен:

уметь:
\begin{itemize}
  \item[--] выполнять мониторинг и анализ работы локальной сети с помощью программно-аппаратных средств;
  \item[--] осуществлять диагностику и поиск неисправностей всех компонентов сети;
  \item[--] выполнять действия по устранению неисправностей.
\end{itemize}


знать:
\begin{itemize}
  \item[--] архитектуру и функции систем управления сетями, стандарты систем управления; средства мониторинга и анализа локальных сетей;
  \item[--] методы устранения неисправностей в технических средствах;
\end{itemize}

КОС промежуточной аттестации имеют своей целью определение сформированности общих и профессиональных компетенций:

\input{competencies.txt}
